...
AWS_S3_ENDPOINT_URL=...
```
Set `DEBUG=True` locally for more verbose console logging. Set `DATABASE_URL` to be the database url of whatever database service you use. The format shown is for MySQL, but adjust accordingly. The values for the `AWS` related variables can be found in `dito_env_variables.txt` in the project Google Drive. 
